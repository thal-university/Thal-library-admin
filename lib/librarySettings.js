 ...
 * @param {Object} [input] - issue_date / due_date for confirm, return_date for return
 */
...
/** Delete a pending reservation (confirmed ones are archived instead). */
export async function deleteReservation(id) {
  return unwrap(await fetch(`/api/reservations/${id}`, { method: 'DELETE' }))
}

/**
 * Read reservations. Runs server-side because row level security hides the
 * table from the browser's anon key.
 * @param {'active'|'archived'|'all'} scope
